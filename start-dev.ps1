...
if ($pg) {
    ...
    Write-Host "[WARN] PostgreSQL not detected - start it from Services if DB errors occur" -ForegroundColor Yellow
}
...
if ($occupied) {
    $backendPort = 8001
    Write-Host "[INFO] Port 8000 occupied - using port 8001 instead" -ForegroundColor Yellow
    Write-Host "       Tip: reboot once to permanently free port 8000" -ForegroundColor DarkGray
}

# Update .env.local files to match chosen port
...
Start-Process "cmd.exe" -ArgumentList "/k title PulseIQ-Backend & `"$UVICORN`" main:app --host 127.0.0.1 --port $backendPort" -WorkingDirectory "$ROOT\backend" -WindowStyle Normal

Start-Sleep -Seconds 3

# -- Start Admin Panel --
Write-Host "Starting Admin Panel (port 3001)..." -ForegroundColor Cyan
Start-Process "cmd.exe" -ArgumentList "/k title PulseIQ-Admin & pnpm dev" -WorkingDirectory "$ROOT\admin" -WindowStyle Normal

# -- Start Frontend --
Write-Host "Starting Frontend (port 3000)..." -ForegroundColor Cyan
Start-Process "cmd.exe" -ArgumentList "/k title PulseIQ-Frontend & pnpm dev" -WorkingDirectory "$ROOT\frontend" -WindowStyle Normal

# -- Start Mobile Web --
Write-Host "Starting Mobile Web (port 3002)..." -ForegroundColor Cyan
Start-Process "cmd.exe" -ArgumentList "/k title PulseIQ-Mobile & pnpm dev" -WorkingDirectory "$ROOT\mobile-web" -WindowStyle Normal
